let shortcuts = [
    {
        title: '当前行变为checklist cmd + l',
        description: '注意是字母l。在行首添加 - [ ]，如果已经是checklist，则会执行toggle动作。',
        keyword: 'macos obsidian checklist todo',
        keys: ['l', 'command'],
        icon: 'icons/obsidian.png'
    },
    {
        title: '添加注释 cmd + /',
        description: '添加注释, 即%% %%',
        keyword: 'macos obsidian comment 注释',
        keys: ['/', 'command'],
        icon: 'icons/obsidian.png'
    },
    {
        title: '插入markdown链接 cmd + k',
        description: '插入[](), 非[[]]',
        keyword: 'macos obsidian markdown link 链接',
        keys: ['k', 'command'],
        icon: 'icons/obsidian.png'
    },
    {
        title: '切换到第一个tab cmd + 1',
        description: '切换到第一个tab',
        keyword: 'macos obsidian 切换 tab 111',
        keys: ['1', 'command'],
        icon: 'icons/obsidian.png'
    },
    {
        title: '切换到第一个tab cmd + 2',
        description: '切换到第二个tab',
        keyword: 'macos obsidian 切换 tab 222',
        keys: ['2', 'command'],
        icon: 'icons/obsidian.png'
    },
    {
        title: '切换到第一个tab cmd + 3',
        description: '切换到第三个tab',
        keyword: 'macos obsidian 切换 tab 333',
        keys: ['3', 'command'],
        icon: 'icons/obsidian.png'
    },
    {
        title: '切换到第一个tab cmd + 4',
        description: '切换到第四个tab',
        keyword: 'macos obsidian 切换 tab 444',
        keys: ['4', 'command'],
        icon: 'icons/obsidian.png'
    },
    {
        title: '切换到第一个tab cmd + 5',
        description: '切换到第五个tab',
        keyword: 'macos obsidian 切换 tab 555',
        keys: ['5', 'command'],
        icon: 'icons/obsidian.png'
    },
    {
        title: '显示笔记列表 cmd + o',
        description: '显示笔记列表',
        keyword: 'macos obsidian list 列表 open',
        keys: ['o', 'command'],
        icon: 'icons/obsidian.png'
    },
    {
        title: '后退 cmd + option + ⬅',
        description: '后退',
        keyword: 'macos obsidian 后退 back',
        keys: ['left', 'command', 'option'],
        icon: 'icons/obsidian.png'
    },
    {
        title: '前进 cmd + option + ⮕',
        description: '前进',
        keyword: 'macos obsidian 前进 forward',
        keys: ['right', 'command', 'option'],
        icon: 'icons/obsidian.png'
    },
    {
        title: '打开设置 cmd + ,',
        description: '打开设置',
        keyword: 'macos obsidian 设置 setting config',
        keys: [',', 'command'],
        icon: 'icons/obsidian.png'
    },
    {
        title: '预览文件 cmd',
        description: '把光标放在文件管理器的某个文件上，弹出浮动窗口显示这个文件的内容。记住按住command键',
        keyword: 'macos obsidian preview 预览',
        keys: ['command'],
        icon: 'icons/obsidian.png'
    },
    {
        title: '删除当前段落 cmd + d',
        description: '删除当前段落',
        keyword: 'macos obsidian delete 删除段落',
        keys: ['d', 'command'],
        icon: 'icons/obsidian.png'
    },
    {
        title: '加粗 cmd + b',
        description: '设置粗体',
        keyword: 'macos obsidian 加粗 粗体',
        keys: ['b', 'command'],
        icon: 'icons/obsidian.png'
    },
    {
        title: '斜体 cmd + i',
        description: '设置斜体',
        keyword: 'macos obsidian 斜体',
        keys: ['i', 'command'],
        icon: 'icons/obsidian.png'
    },
    {
        title: '当前文件查找并替换 cmd + option + f',
        description: '当前文件查找并替换',
        keyword: 'macos obsidian find replace 查找替换',
        keys: ['f', 'command', 'option'],
        icon: 'icons/obsidian.png'
    },
    {
        title: '重新打开上一个被关闭的 tab cmd + shift + t',
        description: '重新打开上一个被关闭的 tab',
        keyword: 'macos obsidian open closed tab last',
        keys: ['t', 'command', 'shift'],
        icon: 'icons/obsidian.png'
    }
]

module.exports = shortcuts